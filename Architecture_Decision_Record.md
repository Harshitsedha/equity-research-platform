...
## ADR-015 — Thesis as an immutable anchor: optional analyst target over mandatory model provenance; freeze facts, project current

*Decided and built in Phase 3a. The `Thesis` is the recorded analyst view that drift (3b) and the report cadence (3c) will be measured against — so this ADR fixes what a thesis is, what it stores, and what it computes.*

**Context.** Phases 0–2 gave us an immutable ledger of facts (`Snapshot`, `ValuationRun`, `Report`) and a mutable `Stock` aggregate composed over them (ADR-013). What was missing is the *recorded judgement*: the fair value an analyst committed to at a point in time, the model run that anchored it, the load-bearing assumptions (each with a tolerance band), and the prose case. Because everything downstream measures *against* this record, it must be a stable standard of truth — not something that silently changes as the model, the data, or the analyst's mind moves. The modelling risk is the usual one (ADR-013): smuggling derived/current state onto the record so it drifts from the facts it cites.

**Decision.**

1. **The thesis is immutable; "active" is a projection (HARD RULE 1/2).** `thesis` and its append-only child `thesis_assumption` carry the same `block_mutation()` trigger the ledger tables use — you never edit a thesis, you record a new one that supersedes it. The `Stock` aggregate gains an append-only `thesis_history`; `active_thesis` is the latest by `recorded_at`, computed on read, never a stored `is_active` flag. The history is kept ordered on insert (mirroring `snapshot_refs`) and projected on load ordered `(recorded_at, id)`, so the active thesis is identical in memory and after a round-trip.

2. **Target optional, provenance mandatory (HARD RULE 4).** `analyst_target` is nullable — the analyst's adjusted view. `anchor_valuation_run_id` is required. `fair_value()` (the only computed value in 3a, a pure read) returns `analyst_target` when set, else the anchored run's frozen output. The model→judgement gap and all drift are computed *projections* belonging to 3b — never stored here.

3. **FREEZE the anchor output as columns; do not project it from the run.** The thesis stores `anchor_value_per_share` and `anchor_snapshot_id` as NOT NULL columns alongside the run ref. The run is immutable, but the DCF *engine is versioned* (`code_version` exists precisely because the model changes) — a thesis's fair value is a historical fact about the decision made, not a view derived from whatever the model is now. Projecting would conflate "the value I committed to" with "a value read from a run." This is **not** an ADR-013 violation: ADR-013 forbids denormalised *current* state, which goes stale; a thesis-time fair value is a *frozen historical fact*, the same category as `Snapshot.inputs` freezing financials or `Report` freezing content. **Freeze facts, project current.** Thesis fair value is a fact.

4. **A record-time integrity assertion keeps the freeze sound (the invariant for #3).** Because the anchor output is frozen rather than read live, it must be recorded *consistent with its provenance*. `SqlThesisRepository.record` — which can read the run — asserts, before writing, that the stored `anchor_value_per_share == anchored_run.result["value_per_share"]` and `anchor_snapshot_id == anchored_run.snapshot_id`; a mismatch raises `ThesisAnchorMismatchError` and nothing is persisted. The two integrity checks live in their correct layers: **anchor ownership** (the run's snapshot belongs to this stock, HARD RULE 3) is a pure domain check in `Stock.record_thesis` (`CrossStockAnchorError`, mutating nothing on rejection); **frozen-anchor-matches-provenance** is the repo guard. Both are tested, including that a tampered anchor value and a tampered anchor snapshot are each rejected at record time, while a matching one persists.

5. **Assumptions are key-paths into `Snapshot.inputs`, stored in a child table.** Each `ThesisAssumption` carries a `name`, a `recorded_value`, a `ToleranceBand` (either bound nullable for one-directional bands; at least one required; `lower ≤ upper`), and a **`metric_key`** — a flat key into a future `Snapshot.inputs`. This reuses the convention `Claim` already uses ("key/path into snapshot.inputs"); `Snapshot.inputs` is a flat `{metric: value}` map, so a key *is* the actual 3b will resolve against. 3a records the key only — it builds **no** resolver (that is 3b). Assumptions live in a child `thesis_assumption` table (not a JSON column), mirroring the `status_transition` pattern — an append-only child under an immutable parent — so each premise is a first-class immutable row 3b resolves individually, and the immutability trigger covers it.

**Alternatives rejected.**
- *Project the anchor output from the run on load (store only the ref).* Rejected per #3 — the versioned engine makes "read from the run" a moving view, not the committed fact; a commitment record must be self-contained across future engine changes.
- *A mutable `is_active` flag / a stored "current thesis".* Rejected — denormalised current state that drifts (ADR-013); "active" is a query over the append-only history.
- *Assumptions as a JSONB column on `thesis`.* Rejected — a child table matches the established `status_transition` shape, gives each premise an addressable immutable row for 3b, and satisfies the per-row immutability guarantee. (JSONB would be simpler to write but loses per-assumption identity and row-level immutability.)
- *A named-metric extractor for assumptions.* Rejected — `Snapshot.inputs` is already a flat metric map, so a key-path is sufficient and needs no metric catalog or domain computation; an extractor would couple the immutable record to a derived-metric layer that belongs to 3b/valuation.

**Consequences.** A thesis is a faithful, self-contained, immutable record of a judgement: its fair value and anchor are frozen facts that cannot drift, guaranteed consistent with the run they cite at record time; its assumptions are addressable immutable rows carrying exactly what 3b needs to resolve them against a future snapshot. "Active" and every gap/drift remain computed projections. 3b can now measure drift against a stable standard of truth without 3a having computed any of it. Out of scope here and deliberately not built: drift computation, update reports / cadence / re-thesis, any LLM or API/UI surfacing of theses.

### Amendment — Phase 3b: drift computation (built)

*Phase 3b builds the resolver and the drift projections this ADR reserved for it. It adds **no migration** (the 3a `thesis` / `thesis_assumption` columns suffice), no API/UI, no LLM, no async, and touches neither the sealed ledger nor the immutability triggers (snapshots are read via the existing `RepositoryPort.get_snapshot`).*

1. **Drift and the model→judgment gap are computed projections, NEVER stored (reaffirmed).** `domain.drift` is pure (stdlib + pydantic + sibling domain modules); `compute_thesis_drift(thesis, current_snapshot_inputs) -> ThesisDriftProjection` computes on demand and persists nothing. 3b reports **raw fact only** — resolved value, in-band / crossed (+ side + magnitude), or unresolved. It does **not** decide significance: thresholds, cadence, and re-thesis triggers remain 3c.

2. **Dual-convention resolver with derived-wins precedence (a reserved namespace).** An assumption's `metric_key` carries **both** conventions: a name in `NUMERIC_METRIC_KEYS` (the derived-metric set, now sourced directly from `compute_metric`'s dispatch table so the two can never drift) is recomputed via the verification harness's `compute_metric`; any other key is resolved structurally as a flat/dotted path into `Snapshot.inputs`. **Derived wins**: a derived-metric name is the reserved meaning of that key. This is sound because ADR-013 already forbids denormalised *current* state — a raw `Snapshot.inputs` field named like a derived metric would be exactly that forbidden shadow — so derived-wins can never mask a legitimate raw field. The resolver reuses the existing verification paths; there is no parallel resolver.

3. **One shared traversal: `resolve_path`.** The dotted-path walk is extracted out of `citation_exists` into `verification.resolve_path(inputs, key) -> (found, value)`, **purely structural** — it returns `(True, None)` for a present-but-null leaf and makes no interpretation. `citation_exists` becomes a thin `found`-only wrapper (semantics frozen byte-for-byte; a characterization test pins `resolve_path(...).found == citation_exists(...)` across dotted / missing / present-but-None keys). The drift resolver layers the null/unknown interpretation on top — so the single domain traversal serves both the citation check and 3b.

4. **`UNRESOLVED` is a first-class state with a reason enum — it never raises and never reads as "no drift".** `DriftStatus` is `IN_BAND` / `CROSSED` / `UNRESOLVED`; an unresolved assumption carries an `UnresolvedReason`: `MISSING_INPUT` (a *known* derived metric whose inputs can't compute it), `KEY_ABSENT` (a non-derived `metric_key` whose path is NOT present in `Snapshot.inputs`, i.e. `found=False` — the §5 vanished/renamed/typo "premise I can no longer check" bucket), or `VALUE_ABSENT` (path IS present, `found=True`, but the value is null / non-numeric — present-but-unusable). `compute_metric` conflates `KEY_ABSENT` and `MISSING_INPUT` under one `MetricError`; the resolver splits them by checking the derived set FIRST, so a failure on a known metric can only be `MISSING_INPUT`. The names were corrected from the original sketch: `PATH_ABSENT` was renamed **`VALUE_ABSENT`** because it was attached to the present-but-null case, contradicting a name that asserts absence; **`KEY_ABSENT`** (was `UNKNOWN_KEY`) now truthfully names the genuinely-absent path. The `(found, value)` interface is purely structural and so cannot distinguish a vanished/renamed key from a typo — they collapse into the single `KEY_ABSENT` reason, which is correct.

5. **Band-authoritative crossing; the recorded delta is a supplementary observation.** Crossing status comes from the frozen `ToleranceBand` **alone**, with strict inequality (a value equal to a bound is in-band) and no silent symmetry: an upper-only band can only be crossed `above`, a lower-only band only `below`, and the crossed side is always reported. `breach_magnitude` (distance past the crossed bound) is set only when crossed; an in-band value instead reports `distance_to_nearest_bound`, a neutral non-negative slack typed distinctly from a breach. Separately, `resolved_value - recorded_value` is reported as `recorded_delta`, **explicitly a supplementary observation, NOT the drift signal** — because (per the 3a recheck below) `recorded_value` is an independent analyst number, not a snapshot-derived figure, so comparing it to the current resolution is informational only.

   *3a recheck (recorded its finding here):* `recorded_value` is captured as an **independent analyst number** — `Stock.record_thesis` takes assumptions as caller-supplied input and never resolves `metric_key` against the anchor snapshot's inputs (3a builds no resolver, per clause 5 above). It is therefore **not** snapshot-derived; no `resolve_assumption(metric_key, anchor.inputs) == recorded_value` identity holds or is asserted, and 3a is unchanged.

6. **Shipped projections: per-assumption drift (1) and the thesis-static model→judgment gap (2).** The gap is `analyst_target - anchor_value_per_share` (both frozen — needs no current data); when `analyst_target` is `None` it is represented as **ABSENT**, not a zero gap, so it never reads as "model and judgment agree". **Model-fair-value drift (3) is deferred** to its own future slice — it needs valuation-run reachability the aggregate deliberately does not expose, and re-running the engine on the current snapshot would couple the domain to the valuation adapter (HARD RULE 1). Adding a valuation-run timeline to the aggregate is explicitly **not** done.

7. **No surface (held).** 3b is pure domain + a projection computed on load, exactly as 3a surfaced nothing. The two-step load (aggregate for the active thesis + `get_snapshot` for the current inputs) and any read path/UI are a separately-scoped future slice — not built or wired here.

### Amendment — Thesis drift surfacing slice (built)

*The separately-scoped read slice clause 7 deferred. It makes 3b's drift visible over HTTP without changing the domain, storing anything, or adding a migration — the same minimalism as 3a/3b. Lands BEFORE 3c and introduces no significance concept.*

1. **A read-only recompute-every-call endpoint; drift is still never stored (ADR-015).** `GET /stocks/{isin}/drift` is a fourth GET on the existing stocks router, reusing the established read shape exactly: the same two dependencies as `get_snapshot_leaf`, the two-step load done **inline** in the handler (no service layer — matching the established pattern), and an explicit `from_*`-mapped DTO (the api layer owns the wire format, HARD RULE 1). The handler reads only — `get_by_isin` + `get_snapshot` — then calls the **unchanged, pure** `compute_thesis_drift`; nothing is written or persisted, and the projection is recomputed on every request. **No write route** (HARD RULE 2 preserved), **no migration, no new column/table/ORM.**

2. **`Cache-Control: no-store` is the transport-level never-frozen guarantee.** Because drift is a projection that must never be cached as if it were stored fact, the response sets `Cache-Control: no-store`. This is the HTTP analog of ADR-015's never-stored rule: a stale cached drift would reintroduce exactly the frozen-current-state the ADR forbids.

3. **The wire contract preserves every honest-state distinction 3b fought for.** Per-assumption: `status` and `unresolved_reason` are **separate fields** (reason non-null iff `UNRESOLVED`, and each of `KEY_ABSENT` / `VALUE_ABSENT` / `MISSING_INPUT` is independently readable as its own string); `breach_magnitude` (past the crossed bound) and `distance_to_nearest_bound` (in-band slack) are **distinct fields**, never merged into one "magnitude" (exactly one non-null when resolved, both null when unresolved); the supplementary `resolved_minus_recorded` is nested under an **`observation`** object so it can never be read as the crossing signal (the band is authoritative). The model→judgment gap carries an explicit **`present` boolean**: `present=false` (no analyst target — ABSENT) serializes distinctly from `present=true, gap=0.0` (target equals the anchor), so a client never reads "no override" as a zero gap. A schema docstring notes the gap is thesis-**static** (it sits beside `assumption_drifts` because that is the projection shape, but it does not move as the snapshot changes).

4. **The envelope identifies BOTH ends of the drift interval.** `ThesisDriftResponse` carries `thesis_recorded_at` + `anchor_snapshot` (the "then", frozen 3a columns — pure reads off the already-loaded thesis/timeline, no new query) and `current_snapshot` (the "now"). `thesis_present` plus a nullable `current_snapshot` keep the degenerate states **separately readable**.

5. **Degenerate inputs are defined results, not errors.** Only an unknown ISIN is `404` (consistent with the sibling routes). All others are `200`: no active thesis → `thesis_present=false` with every thesis/snapshot/drift field null; an active thesis whose current snapshot leaf is absent (`latest_snapshot_ref` is `None` **or** the defensive `get_snapshot` → `None`, same guard as the snapshot-leaf route) → `thesis_present=true`, `current_snapshot=null`, `drift=null`; a present-but-empty/partial snapshot → a full projection with each assumption `UNRESOLVED` carrying its reason.

6. **Scope held against 3c, and API-only.** The route, DTO, and body carry raw fact only — statuses, sides, magnitudes, the gap, the supplementary delta — and **no** threshold, significance, cadence, staleness, or re-thesis concept anywhere. Model-fair-value drift remains deferred (no valuation-run timeline added). The viewer/frontend is deliberately **not** touched this slice; rendering drift in the Next.js viewer is its own future slice, to be scoped against a now-settled wire shape.

## ADR-016 — Drift significance: the verdict layer over raw drift — interpretations compute, commitments freeze

*Decided and built in Phase 3c. 3b made drift a raw, unjudged projection (in-band / crossed + side + magnitude, or unresolved). 3c adds the **verdict**: is this drift big enough to act on? This ADR fixes how raw drift becomes a band, how staleness and coverage compose in, and — crucially — where the significance thresholds live. It adds **no migration** (0006 holds), no persistence, no LLM, no UI; it is a new pure domain module (`domain.significance`) plus a purely additive `significance` object on the existing `GET /stocks/{isin}/drift` response.*

**Context.** Phase 3b deliberately stopped at raw fact and reserved every significance concept for 3c, and the 3c investigation was gated on seeing real magnitudes first. Eyeballing the only seeded thesis (DEMOA) over its synthetic snapshots surfaced the load-bearing constraint: `breach_magnitude` is in **raw metric units and is not comparable across metrics** — a CROSSED `ebit_margin` breach of 0.01 and a revenue breach of 500 000 are "one band-width out" in spirit but differ by seven orders of magnitude in raw value. So significance edges **must be normalized**, and because one-directional bands have no width, the only normalizer that works for every band shape is **fraction-of-crossed-bound**. The live data is one ~4.5% breach: calibration stays deferred, and any band edge proposed now is an honest placeholder, not a calibrated number.

**Decision.**

1. **Assumption-driven by design; composition is a max-severity OR seam.** The verdict bands the **diagnostic only** — per-assumption band drift. Model-fair-value drift (current model output vs the frozen anchor) stays deferred exactly as in 3b: it needs valuation-run reachability the aggregate does not expose, and re-running the engine would couple the domain to the valuation adapter (HARD RULE 1). The thesis-static `ModelJudgmentGap` **passes through** 3b's output untouched but does **not** enter severity composition — it does not move with the snapshot, so it would be the same the day the thesis is born. Composition is written as `overall = max(resolved-assumption severities, staleness)`, an OR seam, so a future valuation-magnitude term slots in as one more `max(...)` operand with zero refactor.

2. **A single universal normalizer + the zero-bound rule.** Every CROSSED breach normalizes as `breach_magnitude / |crossed_bound|` (fraction-of-crossed-bound), declared as **the** convention for all band shapes so an edge means the same thing everywhere. A breach past a `0.0` bound makes the fraction undefined (division by zero); rather than special-case it numerically, the **zero-bound rule** treats crossing a zero floor/ceiling as **categorically significant** → max-severity (`RE_THESIS`) on a one-directional band, with `normalized_breach` reported as `null`. The IN_BAND near-edge check normalizes the slack the same way (`slack / |nearest bound|`); a zero nearest bound has no meaningful normalizer, so near-edge is **skipped** (→ HOLDS). Escalation is **strict** at both edges, mirroring 3b's strict-inequality crossing: a value exactly at `warn_fraction` or `breach_edge` stays on the gentler side.

3. **Directionality is encoded by band shape — no new column, no thesis tag.** A one-directional band already encodes its dangerous side: a floor can only be crossed *below* (compression), a ceiling only *above* (rising). So **any crossing of a one-directional band is `RE_THESIS`-eligible**. A two-sided band does not encode which side is dangerous, so **both** crossings **cap at `REVIEW`** — the confirming/over-shoot direction must never read as `RE_THESIS`. Letting a thesis tag a dangerous side on a two-sided band is **deferred**: it would need a frozen column → a migration → baked judgement, with zero live demand (only `revenue` is two-sided in the fixtures, and it is in-band).

4. **The verdict vocabulary: three ordered severities + an orthogonal coverage flag.** `HOLDS` < `REVIEW` < `RE_THESIS` are the ordered severities; only they participate in the `max` composition. `UNKNOWN` is **not a fourth severity** — it is the coverage outcome. An unresolved premise contributes **no severity** (it cannot escalate) but carries its `UnresolvedReason` verbatim and marks coverage `PARTIAL`. The **coverage invariant** (the most important behaviour, and the most important test): *a clean `HOLDS` requires full coverage.* If coverage is `PARTIAL` and the composed severity would be `HOLDS`, the headline is reported as `UNKNOWN`, never `HOLDS` — a missing input must never read as "no drift". A partial thesis can still be `REVIEW`/`RE_THESIS` from a *resolved* breach; unresolved downgrades confidence, it never escalates severity. (The canonical case is DEMOA's `market_share`, a premise the snapshot schema can never resolve.)

5. **Staleness is orthogonal to magnitude and caps at `REVIEW`.** Thesis age is its own significance dimension. The clock is **injected** via `as_of` (purity preserved — no clock at construction; the API handler passes request-time `now`), measured off the frozen `recorded_at`. Both `age_days` (raw fact) and `quarters_elapsed` (the judged unit — quarters are the business cadence) are reported. Staleness alone trips **`REVIEW`** (an expiry prompt) but **never `RE_THESIS`** (age is not a falsification) — which also spares us a second uncalibrated threshold.

6. **Interpretations compute, commitments freeze: thresholds are recorded platform parameters, NOT frozen on the thesis.** This is the deliberate counterpart to ADR-015's "freeze facts, project current". A `ToleranceBand` is the analyst's recorded *commitment* and stays frozen on the immutable thesis (3a). The significance edges on top of the normalized breach are an *interpretation* — so they compute, injected as a `SignificanceThresholds` value object (`warn_fraction`, `breach_edge`, `stale_quarters`). Because we are still on synthetic fuel, the defaults are honest placeholders carrying their own provenance — `version="uncalibrated-placeholder-v0"`, `calibrated=False` — and that provenance **echoes on every verdict on the wire**, the same discipline as Phase-1 model/prompt/code_version: a reading is reproducible given (thesis + snapshot + injected clock + the echoed threshold-set). **Open question, recorded:** revisit freezing thresholds onto the thesis (for per-thesis reproducibility) once real-data calibration exists; freezing uncalibrated guesses onto immutable records now would be the wrong trade.

7. **Pure projection, zero persistence — the re-thesis trigger logs nothing.** `re_thesis = (overall == RE_THESIS)` is pure-derived and recomputed every call; its three operands (immutable thesis, immutable snapshot, injected clock) are all immutable/derivable, so it is fully reproducible without storage. There is **no event table and no migration** (0006 holds; the next migration, when one is ever needed, is 0007). This reaffirms ADR-015: computed projections are never stored. The audit trail of a human *acting* on a `RE_THESIS` already exists — it is a new superseding thesis in 3a's append-only `thesis_history`, not a stored drift event.

8. **API surface is purely additive; no breaking change.** A new `significance` object sits beside the **unchanged** `drift` body on `GET /stocks/{isin}/drift` (non-null exactly when `drift` is), with the staleness block and the echoed thresholds provenance. Every existing 3b field keeps its type and meaning; a client ignoring `significance` sees the exact 3b contract. The response keeps `Cache-Control: no-store` (significance is a projection too), the handler maps via explicit `from_*` classmethods (the api layer owns the wire format, HARD RULE 1), and the route stays GET-only (HARD RULE 2). `domain.significance` imports only domain siblings (`drift`, `thesis`) — the purity and layering contracts hold.

**Alternatives rejected.**
- *Absolute magnitude edges.* Rejected per the magnitude gate — `breach_magnitude` spans orders of magnitude across metrics; an absolute edge is meaningless. Fraction-of-crossed-bound is the only normalizer that also covers width-less one-directional bands.
- *Freeze thresholds onto the thesis now.* Rejected for this slice — we are uncalibrated; baking guesses onto immutable records is premature. Kept as an explicit open question for calibration time (#6).
- *A dangerous-side tag on two-sided bands.* Rejected — it needs a frozen column → a migration → baked judgement, with zero live demand. Two-sided crossings cap at `REVIEW` instead (#3).
- *Staleness as a magnitude-equivalent that can trip `RE_THESIS`.* Rejected — age is expiry, not falsification; it caps at `REVIEW` (#5).
- *`UNKNOWN` as a fourth severity in the `max`.* Rejected — it would let a missing input *escalate*; instead it is an orthogonal coverage flag that can only *downgrade* a clean HOLDS (#4).
- *Persisting the re-thesis trigger as an event.* Rejected — it is a pure reproducible projection; the human decision is already recorded as a superseding thesis (#7).

**Consequences.** Drift now carries a verdict: each premise bands to `HOLDS`/`REVIEW`/`RE_THESIS` (or `UNKNOWN` when unresolvable), composing by max-severity into one thesis-level verdict that an unresolved premise can never launder into a false HOLDS, with staleness as an orthogonal REVIEW-only prompt and a pure `re_thesis` flag. The verdict is reproducible from immutable inputs plus an echoed, provenance-marked threshold-set, and the whole layer is a computed projection that stores nothing. The composition seam is left open for model-fair-value drift; the calibration question is recorded, not answered. Out of scope and deliberately not built: model-fair-value drift, the drift-in-viewer UI, report cadence / the two-report product, any LLM, ingestion, real-data calibration, the two-sided dangerous-side tag, and any migration.

---

## ADR-017 — Manual-assisted ingestion: a source-agnostic port, a mutable staging buffer, and a gated promote into the immutable ledger

*Decided and built in Ingestion Phase 0. The flagship compute (Phases 0–3c) is sealed but runs entirely on `synthetic_isin` seeds — significance is `calibrated=False` because there is no real fuel. Ingestion is the binding constraint: real NSE fundamentals into immutable `Snapshot`s. This is the first phase since the spine that is inherently messy (external, inconsistent, missing fields, restatements) and that **persists** (migration 0007). This ADR fixes the ingestion contract, the schema it targets, the honest-gap rule, and the one irreversible boundary.*

**Context.** Everything downstream measures against frozen `Snapshot.inputs`, and the drift/significance stack is starved of real data. Ingestion must turn a messy filing into validated numbers and freeze them — but the ledger is immutable and content-addressed (ADR-003), so an unvalidated or fabricated number that reaches it is permanent and poisons every projection above it. The investigation surfaced a load-bearing conflict and a wrong-shaped incumbent (both resolved below). Manual-assisted is deliberately first: it is pure upload/paste with **zero external network** (ADR-012), so it exercises the whole port + pipeline + promote gate without the network surface Screener will need.

**Decision.**

1. **A source-agnostic port; one fat pipeline; thin adapters.** `IngestionSourceAdapter.read(payload) -> RawExtraction` is the stable port (`domain.ports.ingestion`). An adapter's ONLY job is to turn a source blob into a neutral `RawExtraction` (`source_kind`, `source_doc_ref`, `raw_payload`, `raw_fields`, `claimed_period`) — no canonical mapping, no validation, no persistence. The fat, shared, mostly-pure pipeline (`domain.ingestion`: `normalize → validate`; `storage`: the gated `promote`) is source-agnostic. **Screener is a future drop-in** — one more `read` implementation behind the same port, with zero pipeline change — and is out of scope here.

2. **The legacy `DocumentPort` is wrong-shaped and marked for retirement, not extended.** The Phase-0/1 `DocumentPort` / `ParsedDocument` / `FileDocumentAdapter` raises on absence (cannot represent an honest gap), writes a derived-named key (`ebit_margin`) as a raw input, and has no staging/promote gate. The new `IngestionSourceAdapter` supersedes it for fundamentals; the legacy trio is retired debt (ADR-013 §5 already names "ingestion upsert" as a retirement consumer).

3. **The schema resolution — 8 raw fundamentals, six forbidden derived keys (the load-bearing decision).** `breach_magnitude` is in raw metric units and the metrics resolver uses **derived-wins precedence** (ADR-015 3b §2), so a raw `Snapshot.inputs` field named like a derived metric is the **ADR-013 forbidden shadow** — the resolver recomputes it and silently ignores the ingested value. Ingestion therefore targets ONLY the eight genuinely-raw fundamentals `{revenue, prev_revenue, ebit, net_income, equity, total_debt, total_assets, current_liabilities}` (the union of every raw input the derived metrics require) and is **forbidden from writing any of the six `NUMERIC_METRIC_KEYS`** (`ebit_margin, net_margin, roce, roe, debt_equity, revenue_growth`) as a raw input — a hard block at both validate and promote. **The DCF wart is flagged, not fixed:** the DCF engine reads forward-assumption inputs (`ebit_margin`-as-projection, `tax_rate`, `capex`, `depreciation`, `change_in_nwc`, `net_debt`, `shares_outstanding`) directly from `Snapshot.inputs`; those are modelling assumptions that semantically belong on `ValuationRun.assumptions`. Reconciling that is a separate slice — ingestion simply never writes them.

4. **Absence is never zero (the rule a fabricated value would poison everything through).** A missing fundamental is **omitted from `inputs`** — never zero-filled, never null-as-data — so the resolver reports the real gap (`KEY_ABSENT` / `VALUE_ABSENT` / `MISSING_INPUT`). This is **structural, not a convention**: the manual form represents an absent field as `None`, a state distinct from the string `"0"` (a real zero), so absence is *unrepresentable as zero by construction* (decision 3). `normalize` drops `None`/unmapped fields; a present-but-unparseable value (e.g. `"N/A"`) is *wrong* and hard-blocks. A fabricated zero would make `ebit=0 → ebit_margin=0.0` resolve as a real breach — the exact silent poisoning this rule exists to prevent.

5. **A mutable staging buffer (`ingestion_draft`) — the ONE table deliberately not `block_mutation`'d.** Normalize, human correction, and re-validation all mutate the draft freely; the immutable ledger never sees an unvalidated number. The draft retains the raw payload + its hash, the **declared `present_values`** (the editable canonical-key→raw-string surface — the gate's source of truth, so a pre-stripped float cache can never hide a bad value from it), the validated `proposed_inputs` cache, the live validation results, source provenance, and a lifecycle `status`. Manual-assisted is propose-and-confirm: the pipeline proposes a field→canonical-key mapping, the **human confirms/corrects in the draft** (the human is the ground-truth oracle this phase exists to provide), and a manual draft is **never auto-promoted**.

6. **The promote gate — the single irreversible boundary (gate or nothing).** `promote` runs the full gate and, only if every assertion holds, writes the immutable `Snapshot` (via the pure `freeze_snapshot` ledger factory) AND its `snapshot_source` row in ONE transaction, then flips the draft to `promoted`. It asserts: (1) no derived-named key (ADR-013); (2) every present value numeric; (3) period identity valid; (4) source provenance complete; (5) the **reproducibility gate** — the retained raw payload re-hashes to the stored `raw_payload_hash` AND `content_hash` recomputes via `freeze_snapshot` inside the txn. "Reproducible from its retained source" is thus a **structural guarantee at promote, not a later hope** — the ingestion analog of verify-or-fail. Any failed assertion raises `PromotionBlocked` before any write, mirroring `save_report`'s `NonStorableReportError` boundary guard. Promote is **one-way** (a non-draft draft is refused, idempotent-safe); irreversibility lives in `block_mutation()`, not in app code. The stock side resolves through the aggregate (`SqlStockRepository`), never the legacy DTO (ADR-013 §5).

7. **Source provenance as an immutable side table (`snapshot_source`), not new columns on `snapshot` (decision 1).** A 1:1 FK→`snapshot`, written in the same promote txn, immutable via the same `block_mutation()` trigger. It carries `source_kind` (`manual_paste`/`screener`/`filing`), `source_doc_ref`, `raw_payload_hash` (the reproducibility anchor), `ingested_at`, `ingested_by`, `ingestion_code_version`, and `supersedes_snapshot_id` (the restatement chain). The **considered alternative** — nullable provenance columns on `snapshot`, the exact Phase-1 `Report` mirror — was rejected to keep the sealed, hot immutable `snapshot` row and its frozen contract **untouched** (synthetic seeds and all Phase-0/1 snapshots predate ingestion and would carry nulls anyway); the side table is still frozen-with-the-fact and queryable on its own.

8. **Restatement is a new superseding snapshot, never a mutation.** A restatement re-ingests the same `(stock_id, as_of, kind)` and promotes a new immutable `snapshot` (later id) with `supersedes_snapshot_id` set to the prior; "latest for the period" is the **max-id query** (no unique constraint on the period tuple — Finding 1). Consistent with the append-only spine and exactly how the drift stack already resolves "current".

**Alternatives rejected.**
- *Extend `DocumentPort`.* Rejected per #2 — it is wrong-shaped (raises on absence, writes a derived key, no gate). New port; legacy retired.
- *Write the DCF forward-assumption schema (incl. `ebit_margin`) as raw inputs.* Rejected per #3 — that is the ADR-013 forbidden shadow; ingestion targets the eight genuinely-raw keys and the DCF wart is flagged for a separate slice.
- *Zero-fill or null-fill a missing fundamental.* Rejected per #4 — it fabricates a fact and silently poisons the drift/significance stack; absence is structural omission.
- *Validate-then-promote off a stripped float cache.* Rejected — a stripped cache hides bad values from the gate; the gate re-validates the declared `present_values` (#5).
- *Provenance as nullable columns on `snapshot`.* Considered and rejected per #7 — keeps the sealed immutable row untouched; the side table is the cleaner home.
- *A `block_mutation` trigger on `ingestion_draft`.* Rejected — staging is mutable by design; immutability begins at promote.

**Consequences.** Real NSE fundamentals can now enter the immutable ledger through a source-agnostic port and a gated, reproducible promote, with Screener a future drop-in adapter. A missing field stays an honest absence the whole drift/significance stack reads truthfully, never a fabricated zero. The promoted snapshot and its source provenance are immutable and reproducible from a retained-payload hash; restatements append rather than mutate. Migration 0007 adds the mutable `ingestion_draft` and the immutable `snapshot_source`, leaving the `snapshot` row and the shared `block_mutation()` function untouched; it must be applied manually at deploy (it will not auto-apply — the same gap as 0/1/2). Out of scope and deliberately not built: Screener / scraping / any external network, scheduling/cadence, vendor APIs, XBRL/PDF parsing, LLM-assisted extraction, the drift-in-viewer UI, the report product, real-data calibration, and the DCF forward-assumption reconciliation (flagged, not fixed). The first real ISIN goes through manual-assisted as the live validation gate — the calibration fuel this phase exists to produce.
