...
4. **`UNRESOLVED` is a first-class state with a reason enum — it never raises and never reads as "no drift".** `DriftStatus` is `IN_BAND` / `CROSSED` / `UNRESOLVED`; an unresolved assumption carries an `UnresolvedReason`: `MISSING_INPUT` (a *known* derived metric whose inputs can't compute it), `UNKNOWN_KEY` (neither a derived metric nor a resolvable path), or `PATH_ABSENT` (path present in structure but the value is null / non-numeric — present-but-no-value). `compute_metric` conflates the first two under one `MetricError`; the resolver splits them by checking the derived set FIRST, so a failure on a known metric can only be `MISSING_INPUT`. (Refinement vs the original sketch: at `resolve_path`'s `(found, value)` interface, a not-found key is observably indistinguishable from a "path not present" one, so not-found maps to `UNKNOWN_KEY` and `PATH_ABSENT` is reserved for the present-but-null/non-numeric case the interface *can* distinguish.)
